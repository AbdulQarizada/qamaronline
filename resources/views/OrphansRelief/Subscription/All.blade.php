...
<div class="row">
    <div class="col-md-4 col-sm-12 mb-2">
        <div class="hstack gap-3">
            ...
            <select class="form-select  form-select-lg @error('Country') is-invalid @enderror" onchange="window.location.href = this.value;">
                <option value="{{ route('AllSubscription') }}">Please Filter Your Choices</option>
                <option value="{{ route('AllSubscription') }}" {{ $PageInfo == 'All' ? 'selected' : '' }}>All</option>
                <option value="{{ route('ActiveSubscription') }}" {{ $PageInfo == 'Active' ? 'selected' : '' }}>Active</option>
                <option value="{{ route('InActiveSubscription') }}" {{ $PageInfo == 'InActive' ? 'selected' : '' }}>InActive</option>
            </select>
        </div>
    </div>
    <div class="col-md-8 col-sm-12 mb-2">
        ...
        <a href="{{ route('CreateSubscription') }}"  class="btn btn-outline-success btn-lg waves-effect  waves-light float-end btn-rounded text-uppercase"><i class="mdi mdi-plus me-1"></i>ADD Subscription</a>
    </div>
</div>
<div class="row">
    <div class="col-12">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div>
                                @if ($data->orphan)
                                <h5 class="font-size-14 mb-1"><a href="#" class="text-dark">{{ $data->orphan->FirstName }}</a></h5>
                                <p class="text-muted mb-0">TazkiraID: <a href="#" class="text-dark">
                                        {{ $data->orphan->TazkiraID }}</a></p>
                                @endif
                            </div>
                        </td>
                        <td>
                        ...
                        <td>
                            <div>
                                <p class="text-muted mb-0 badge badge-soft-warning">{{ $data->Amount }}$</p>
                                <h5 class="font-size-14 mb-1"><a href="#" class="text-dark badge badge-soft-primary">{{ $data->Type }}</a></h5>
                            </div>
                        </td>
                        <td>
                            <div>
                                <p class="text-muted mb-0"><a href="#" class="text-success">
                                        {{ Carbon\Carbon::parse($data->StartDate)->format('j F Y') }}</a></p>
                                <p class="text-muted mb-0"><a href="#" class="text-danger">
                                        {{ Carbon\Carbon::parse($data->EndDate)->format('j F Y') }}</a></p>
                            </div>
                        </td>
                        <td>
                            @if ($data->IsActive != 1)
                        ...
                        <td>
                            @if ($data->Created_By != '')
                            <div>
                                <h5 class="font-size-14 mb-1"><a href="#" class="text-dark">{{ $data->UFirstName }}
                                        {{ $data->ULastName }}</a></h5>
                        ...
                        <td>
                            <div class="d-flex flex-wrap gap-2">
                                @if ($data->IsActive == 1)
                                <a href="{{ route('DeActivateSubscription', ['data' => $data->id]) }}" class="btn btn-sm btn-outline-danger waves-effect DeActivateCard waves-light" data-bs-toggle="tooltip" data-bs-placement="top" title="De-Activate Subscription">
                                    <i class="mdi mdi-account-tie-voice-off-outline font-size-16 align-middle"></i>
                                </a>
                                @endif
                                @if ($data->IsActive != 1)
                                <a href="{{ route('ActivateSubscription', ['data' => $data->id]) }}" class="btn btn-sm btn-outline-success waves-effect ActivateCard waves-light" data-bs-toggle="tooltip" data-bs-placement="top" title="Activate Subscription">
                                    <i class="mdi mdi-account-tie-voice-outline font-size-16 align-middle"></i>
                                </a>
                                <a href="{{route('EditSubscription', ['data' => $data -> id])}}" class="btn btn-sm btn-outline-info waves-effect waves-light" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit Details">
                                    <i class="mdi mdi-square-edit-outline font-size-16 align-middle"></i>
                                </a>
                                <a href="{{ route('DeleteSubscription', ['data' => $data->id]) }}" class="btn btn-sm btn-outline-danger waves-effect waves-light delete-confirm" data-bs-toggle="tooltip" data-bs-placement="top" title="Delete Record">
...
<div class="row">
    <div class="col-lg-12">
        <ul class="pagination pagination-rounded justify-content-center mt-3 mb-4 pb-1">
            {!! $datas->links() !!} <span class="m-2 text-white badge bg-dark">{{ $datas->total() }} Total
                Records</span>
        </ul>
    </div>
</div>
